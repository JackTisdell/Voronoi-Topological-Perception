...
nObj=2;
radObj=pi/2; %geodesic radius of the spherical cap on which we place the objectives

N=Nr+Nl; %total number of agents;
%% Parameters of the model
%L=3.46*2*sqrt(pi)/sqrt(N); %Euclidian Lengthscale
L=.87*acos(1-2*pi/N); %Spherical Lengthscale
...
MomentAng_t=zeros(kMax,1);
MomentAngAbs_t=zeros(kMax,1);
...
%% Specifying the objectives location

if nObj==1
    objective=[1;0;0]; % %intersection of x-axis and S^2.
else
    azanglObj=(0:nObj-1)*2*pi/nObj; %azimutal angles of the objectives w.r.t the x-axis.
    polanglObj=radObj*ones(1,nObj); %polar angle of the objectives w.r.t the x-axis.
    objective=zeros(3,nObj); %DO NOT REMOVE THIS PRE-ALLOCATION OF MEMORY, OTHERWISE YOU'LL HAVE PROBLEM WHEN CHANGING THE VALUE OF nObj
    %NOTE: The polar axis is taken to be the x-axis (and not the z-axis as
    %usual...that's why the formulas for the change spherical-->cartesian
    %look off.
    objective(1,:)=cos(polanglObj);
    objective(2,:)=sin(polanglObj).*cos(azanglObj);
    objective(3,:)=sin(polanglObj).*sin(azanglObj);
end
...
%% Homming component h^(q)
x=XYZ(:,1); y=XYZ(:,2); z=XYZ(:,3);
Hxyz=zeros(N,3);

dist=sqrt((x-objective(1,:)).^2+ (y-objective(2,:)).^2+(z-objective(3,:)).^2); % N*nObj matrix obtained by outer difference giving the distance to each objective from each generator
[distEucl_closestObj,idx_closestObj]=min(dist,[],2);

if q==Inf % if the target's field of view is infinite then all the agents move towards their closest objective
    idx_h=idx_closestObj;  %the index of objective retained to move towards to

else % if q<Inf then the only agents attracted towards the objectives are those located at a topological distance not bigger than q.
     % REMARK: It is possible that a particular generator is within the q-topo
     % neighborhood of several objectives...in that case the tie breaking rule is
     % given by the closest target
    
    %1) compute the closest agents to each objective point.
    [distEucl_nearestToObj,idx_nearestToObj]=min(dist); %index of agents respectively closer to each of the objectives as well as that distance.
    
    %2) % the topological q-neighborhood of the agents respectively closest to each of the objective.
    cand_moving2objective=idx_topoField(idx_nearestToObj); % a nObj*1 cell array, e.g.n cand_moving2objective{2} contains the index of agents that are in the q-topo field of view of Objective 2
    
    
    % 3) to properly assigning an objective to the agents in
    % the intersection of two or more candidates lists we proceed as follow:
    %
    % - we find the agents that are in more than two different q-topo neigh
    % - for these ambiguous agents, we find the closest objective AMONG
    %   THOSE WHO INCLUDED THE AGENT IN THEIR q-topo neigh.
    % 
    % NOTE: the last constraint is to ensure we do not loose any
    % topological information on these ambiguous agents; because an agent
    % might be in the q-topo neighborhood of, say, two different objectives
    % but its closest (metric wise) objective is a third one.
    
    %Immediate assignment of an objective to each agent that should move.
    %Note that because the cells of cand_moving2objective are not
    %necessarely mutually disjoint, some of the entries of idx_h will be
    %visited and modified several times. We will counter that afterwards.
    idx_h=NaN(N,1);
    for nn=1:nObj %for each objective we compute the homing component for all agents within the p-topo neighborhood 
        idx_h(cand_moving2objective{nn})=nn; 
    end
    
    %Now we take care of the ambiguous agents and make sure to assign them
    %the correct objective.
    cand_moving2objective_concat=vertcat(cand_moving2objective{:}); %concatenated list of agents that fall in the q-topo neigh of at least one of the objectives
    [~,ia,~]=unique(cand_moving2objective_concat,'stable'); %indices of agents who are unrepeated
    
    if length(ia)~=length(cand_moving2objective_concat) % if true then we know there are ambiguous agents, othwerwise there are no agents belonging to the q-topo field of two or more objectives
        
        ib=(1:length(cand_moving2objective_concat))';
        idx_duplicate=cand_moving2objective_concat(setdiff(ib,ia)); % indices w.r.t [x,y] of the agents that belong to the q-topo field of two or more objectives
        edges=[1;cumsum(cellfun(@length,cand_moving2objective))+0.1]; % artifact that will help us know to which objectives belong the ambiguous agents 
        
        for ll=1:length(idx_duplicate)
            idx=find(cand_moving2objective_concat==idx_duplicate(ll));
            idx_obj_cand=discretize(idx,edges);
            [~,idx_min]=min(dist(idx_duplicate(ll),idx_obj_cand));
            idx_obj_good=idx_obj_cand(idx_min);
            
            idx_h(idx_duplicate(ll))=idx_obj_good; %this finally solves the ambiguity between agents that are within the q-topo field of several objectives.
...
%At this point idx_h contains the correct objective that each agent
%needs to move towards to, i.e. idx_h is an N*1 array whose entries
%belong to 1:nObj if q=Inf. If q~=Inf then some agents may have a
%corresponding NaN entry, meaning that those agents are not attracted
%towards any objective.
%Next we extract the index of agents that will have a nonzero homing
%component as well as the objective they will move towards to.
idx_agent_homing=find(~isnan(idx_h)); idx_h=idx_h(idx_agent_homing);

% We now need to compute the Euclidian displacement vectors and project
% them onto the tangent planes of each agent
Hxyz(idx_agent_homing,:)=objective(:,idx_h)'-XYZ(idx_agent_homing,:); %Euclidian vector pointing AWAY from the closest neighbor
Hxyz=Hxyz-dot(Hxyz',XYZ')'.*XYZ; %Projection of the Euclidian vectors onto the tangent planes.
                                % Already verified that dot(Hxyz',XYZ')'=0
                                % for agents whose idx_h is not NaN.
                                % Thus the vectors Hxyz belong
                                % indeed to the tangent planes after the
                                % last update.
nor=sqrt(sum(Hxyz.^2,2)); %we normalize the homing vectors
idx_nor_small=find(nor<=1e-10);
nor(idx_nor_small)=ones(size(idx_nor_small));
Hxyz(idx_nor_small,:)=zeros(length(idx_nor_small),3);
Hxyz=Hxyz./nor;
...
Gamma_dot=((1-r_L).*Hxyz+r_L.*Uxyz+nu*Axyz_tilda)/(1+nu);
...
%             Earth.View=[151.4 23.8000];
        ...
        scatter3(1.01*objective(1,:),1.01*objective(2,:),1.01*objective(3,:),50,'MarkerEdgeColor',[0 .6 0],'MarkerFaceColor',[0 .6 0]) %objectives
...
%b) Angular momentums, shifted w.r.t to a point on the sphere...WLOG we
%take that point to be the first objective
shiftXYZ=XYZ-objective(:,1)'; %shiftet position w.r.t the first objecive
nor_shiftXYZ=sqrt(sum(shiftXYZ.^2,2));
shiftAng_Vel=cross(shiftXYZ,Vel); %vectors of shifted Angular velocities, i.e. computed w.r.t the first objective
MomentAng_t(k)=sqrt(sum(sum(shiftAng_Vel).^2))/sum(nor_shiftXYZ.*nor_Vel);
MomentAngAbs_t(k)=sum(sqrt(sum(shiftAng_Vel.^2,2)))/sum(nor_shiftXYZ.*nor_Vel);
...
save("data.mat",'dt','objective','Nr','Nl','L','nu','q','p','DT_t','V_t','C_t','XYZ_t','Vel_t','Vel_old_t','Loc_alignment_t','Polarization_t','MomentAng_t','MomentAngAbs_t','Area_t','Energy_t','AngleVel_t','AvgVel_t','StdVel_t');
