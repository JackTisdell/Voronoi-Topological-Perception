...
% We shift the vertices by [meanx;meany] to improve accuracy
%
x = x - mean(x);
y = y - mean(y);
...
A = sum( a ) /2; %signed area
% xc = sum( (x+xp).*a  ) /6/A; %centroid coordinates
% yc = sum( (y+yp).*a  ) /6/A;
Ixx = sum( (y.*y +y.*yp + yp.*yp).*a  ) /12;  % second moments of inertia
Iyy = sum( (x.*x +x.*xp + xp.*xp).*a  ) /12;


% In case we have a CW order we change the sign
A=abs(A);
